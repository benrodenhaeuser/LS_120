...
class Computer < Player
  ...
  def choose_randomly(board, other_player)
    board << Move.new(board.available_squares.sample, self)
  end

  def choose_optimally(board, other_player)
    nil # todo
  end

  # this algorithm needs information about the players, including "opponent_of".
  # given that we have the other player and the board, we should be fine.
  def nega_max(player, state, top = false)
    if terminal?(state) # Board class
      best_value = payoff(player, state) # add to Board class
    else
      best = available_moves(state).map do |move| # Board class
        make(move, player, state) # append to Board
        value_for_move = -(nega_max(opponent(player), state))
        unmake(move, state) # pop from Board
        [move, value_for_move]
...
class TTTGame
  ...
  def play_round
    loop do
      active_player.choose(board, opponent_of(active_player))
      display_position
      evaluate_winner
