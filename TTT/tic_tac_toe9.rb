module TTT
  ...
  module MessageToUser
    ...
    def self.display_settings(args)
      ...
      prompt("You are the #{args[:human_color]}-player.")
      prompt("The computer is the #{args[:computer_color]}-player.")
      prompt("The #{args[:starting_color]}-player kicks off each round.")
      prompt("The computer skill level is #{args[:skill_level]}.")
      prompt("(1: 'dumb', 2: 'intermediate', 3: 'very smart')")
      prompt("It takes #{args[:rounds_to_win]} round win(s) to win the match.")
    end
  ...
  class Settings
    ...
    attr_accessor :x_color, :o_color, :colors, :human_color, :computer_color,
                  :rounds_to_win, :skill_level, :starting_color

    def initialize
      # not (yet) customizable by user
      @x_color = X_COLOR
      @o_color = O_COLOR
  ...
  class Match
    def initialize(args)
      ...
      @schedule = Schedule.new(human, computer, player(args[:starting_color]))
  ...
  class Game
    ...
    def configure
      ...
      settings.customize if GetUserInput.customize?
      MessageToUser.display_settings(settings.to_h)
      GetUserInput.press_enter
    end
